...
const handleClick = target => {
  ...
  if (
    answer.style.maxHeight
      ? setStyle(answer, { maxHeight: null }, 'hide')
      : setStyle(answer, { maxHeight: `${answer.scrollHeight + 40}px` }, 'show')
  );
};
...
const setStyle = (div, props, state) => {
  if (
    state === 'show'
      ? div.classList.add('faq__answer--show')
      : div.classList.remove('faq__answer--show')
  );
